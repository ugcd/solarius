...
The procedure of SNPs annotation is possible if the user loads `NCBI2R` R package.
...
Annotation of the synthetic SNPs generated for `dat30` data sets fails,
since the names expected by `NCBI2R` R package must begin with *rs* symbols.
...
tab <- annotate(snps, capture.output = TRUE)
tab
```
