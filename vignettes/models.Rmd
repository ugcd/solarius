...
The model includes the fixed effects ($\beta$) 
...
such as `id` individual ID, `sex` gender, `fam` family ID, `fa` father ID, `mo` mother ID,
and `mztwin` indicator of identical individuals, such as monozygotic twins or triplets
...
The second $c$ term represents any annotated environmental effect.
...
For the house-hold grouping the pedigree data must contain a house-hold ID field,
