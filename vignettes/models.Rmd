...
The model is composed of the fixed effects ($\beta$) 
...
`SOLAR` estimates the empirical kinship matrix based on the ID fields in the pedigree data,
such as `id` sample ID, `sex` gender, `fam` family id, `fa` father id, `mo` mother id,
and `mztwin` id
...
The second $c$ term represents any shared environmental effect.
...
For the hose-hold group analysis the pedigree data must contain a house-hold ID field,
and the expected field name is `hhid`.
By default, `SOLAR` creates `Pedigree-Household Groups` based on the `hhid` identifiers.
That means that pedigrees and household groups are merged into larger groups 
that contain every individual in the same pedigree as well as every individual that has the same 
...
returns the covariate-free phenotype, expressed as $y - X \beta$,
