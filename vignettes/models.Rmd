...
the installation files are unpacked, there are `README` file and `install_solar` script, which installs the program. 
...
`solarius` package is not yet in the CRAN repository for R packages [http://cran.r-project.org/](http://cran.r-project.org/),
so it can not be install by calling the usual R function `install.packages`.
Instead, the R package `devtools` allows to install any package hosted in the GitHub web page [https://github.com](https://github.com).
...
* Simulated data from the R package `multic` [@Andrade2006] (polygenic and linkage models);
* Simulated data `dat50` from the R package [FFBSKAT](http://mga.bionet.nsc.ru/soft/FFBSKAT/) (polygenic model with custom kinship matrix and association model);
* GAW10 data distributed within `SOLAR` (polygenic and linkage models).

The simulated data from `multic` R package 
are distributed in `SOLAR` format 
in [inst/extdata/solarOutput](https://github.com/ugcd/solarius/tree/master/inst/extdata/solarOutput)
directory of `solarius` package.
12000 individuals in 200 families are included in the study.
...
For the linkage analysis 6 IBD matrices for chromosome 5 are are available from `multic` package.
These six matrices are duplicated for chromosome 2 in `solarius`,
in order to test parallel calculation (separated by chromosome) of the linkage model.
...
One can see that both unrelated and related individuals are presented in `dat50` data set.
...
a trait with a _poisoned_ distribution `exp_trait1` is created in `dat30` data set.
...
The list of available transforms in `soalrius` 
...
and its diffrent transformations.
...
It might happe that traits in the multivariate polygenic analysis 
...
The association analysis is demonstrated 
on `dat50` data set with 66 individuals and 50 SNP variants.
...
The values of the SNP covariates are any numeric numbers,
that means the dosage format of the imputed SNP data can be accepted.
...
Both arguments `genocov.files` and `snplists.files` are required by `SOLAR`.
...
The function looks for multipoint IBD matrices in directory pointed out by `mibddir` argument.
The analysis can be customized by combination of options via 
`multipoint.options` and `multipoint.settings` arguments.

Phenotypes in `dat30` data set and IBD matrices in a directory distributed withing `solarius`
are used for demonstration of the linkage analysis.
...
In particular, the linkage is run on chromosome 5 with 2 cM interval,
fine mapping is disabled and the second pass will be conducted,
if the maximum LOD score will be higher than 3.
...
The LOD scores for the second path (stored in `lodf2`) are plotted in the code given below.
...
The bivariate linkage analysis is performed in the same way as the bivariate linkage analysis.
